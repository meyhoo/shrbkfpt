(function($,layer){
	...
	clds_layer = {
		...
		alert: function(a, b, c, d){
			...
			if(b=="info"){
				layer.alert(a, {icon:1,skin:"layui-layer-style2",cancel:cancelFun}, function(index){
					callback();
					layer.close(index);
				});
			}else if(b=="error"){
				layer.alert(a, {icon:2,skin:"layui-layer-style2",cancel:cancelFun}, function(index){
					callback();
					layer.close(index);
				});
			}else if(b=="warn"){
				layer.alert(a, {icon:7,skin:"layui-layer-style2",cancel:cancelFun}, function(index){
					callback();
					layer.close(index);
				});
			}
