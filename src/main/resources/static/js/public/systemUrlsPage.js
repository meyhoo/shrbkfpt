...
window.tableObj = $('#tableList').DataTable({
    ...
    "ajax":{						//ajax请求地址
        "url" : window.baseUrl + '/public/searchSystemUrlList',
        "type" : "post",
        "data": function ( req ) {
    ...
    "columns":[
        ...
        { title:'环境清单名', name: 'systemUrlId', data: 'systemUrlId' },
        { title:'中文说明', name: 'systemUrlInfo', data: 'systemUrlInfo' },
        { title:'SIT', name: 'devUrl', data: 'devUrl' },
        { title:'UAT', name: 'testUrl', data: 'testUrl' },
        { title:'准生产', name: 'zscUrl', data: 'zscUrl' },
        { title:'灾备', name: 'zbUrl', data: 'zbUrl' },
        { title:'生产', name: 'prdUrl', data: 'prdUrl' },
        { title:'操作', name: 'opration', data: null,"render": function ( data, type, row ) {
                return 	"<a class='able-a' ms-click=\"changeDialog('edit', "+row.id+")\">修改</a>"+
                    "<a class='able-a' ms-click=\"openDelete(\'"+row.systemUrlId+"\')\">删除</a>";
            }}
    ],
...
window.vmcontent = avalon.define({
    ...
    resetAction: function(){
        window.vmcontent.sendParams.systemUrlId = "";
    },
    ...
    doDeletes: function(){
        ...
        $selected.each(function(){
            ...
            selectedIds.push(data.systemUrlId);
        });
        layer.confirm('删除后无法恢复，确定要删除所选元素吗？', {
            ...
            $.post(
                window.baseUrl + '/public/deleteSystemUrls',
                {
                    systemUrls: JSON.stringify(selectedIds)
                },
                function(res){
    ...
    openDelete: function(param){
        layer.confirm('删除后无法恢复，确定要删除该单位吗？', {
            ...
            $.post(
                window.baseUrl + '/public/deleteSystemUrl',
                {
                    systemUrlId:param
                },
                function (res) {
    ...
    changeDialog: function(t, id) {
        if(t == "add"){
            $("#subFrame").attr("src", window.baseUrl + "/public/addSystemUrlPage.html");
        }else if(t =="edit"){
            window.editData = window.tableObj.row($(this).parent().parent()).data();
            $("#subFrame").attr("src", window.baseUrl + "/public/editSystemUrlPage.html");
        }
        if(t == "main"){
