...
window.tableObj = $('#tableList').DataTable({
    ...
    "ajax":{						//ajax请求地址
        "url" : window.baseUrl + '/developer/searchAdministratorVersionList',
        "type" : "post",
        "data": function ( req ) {
    ...
    "columns":[
        ...
        { title:'操作', name: 'opration', data: null,"render": function ( data, type, row ) {
                if(row.committerFlag=='0'){
                    return 	"<a class='able-a' ms-click=\"becomeCommitter(\'"+row.versionId+"\')\">成为提交者</a>";
                }
                if(row.committerFlag=='1'){
                    return 	"<a class='able-a' ms-click=\"changeDialog('commit')\">提交版本</a>";
                }
            }}
    ],
...
window.vmcontent = avalon.define({
    ...
    becomeCommitter: function(id) {
        layer.confirm('确认要成为该版本的提交者吗？', {
            title: '提示',
            skin: 'layui-layer-style1',
            btn: ['确定','取消'] //按钮
        }, function(){
            $.post(
                window.baseUrl + '/developer/becomeCommitter',
                {
                    versionId:id
                },
                function (res) {
                    if (res.errorCode=='000000') {
                        clds_layer.msg("操作成功！", "info");
                        window.tableObj.ajax.reload();
                    } else {
    ...
    changeDialog: function(t) {
        if(t=="commit"){
            ...
            $("#subFrame").attr("src", window.baseUrl + "/developer/commitVersionPage.html");
        }
        if(t == "main"){
